#!/usr/bin/env bash
# One internal provider-backed smoke case, never performance evidence.
# BENCHMARK_* and the JSON benchmark field are legacy compatibility aliases.
#
...
name="${1:-hewo}"
task_file="${2:-$root/.agents/development/hewo/cases/tasks/example-task.md}"
...
run_dir="${CASE_RUN_DIR:-${BENCHMARK_RUN_DIR:-$root/artifacts/case-$(date +%Y%m%d-%H%M%S)-$$}}"
workspace="${CASE_WORKSPACE:-${BENCHMARK_WORKSPACE:-${E2E_WORKSPACE:-$run_dir/workspace}}}"
CASE_API_KEY_ENV="${CASE_API_KEY_ENV:-${BENCHMARK_API_KEY_ENV:-}}"
...
if [[ -f "$root/.frozen.json" ]]; then
  ...
  snapshot_root="$(mktemp -d "${TMPDIR:-/tmp}/agent-case.XXXXXX")"
  while IFS='=' read -r key value; do
    case "$key" in
...
if [[ -n "${CASE_API_KEY_ENV:-}" ]]; then
  [[ "$CASE_API_KEY_ENV" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || { echo "invalid case API key environment" >&2; exit 2; }
  [[ -n "${!CASE_API_KEY_ENV:-}" ]] || { echo "$CASE_API_KEY_ENV is not set" >&2; exit 2; }
  auth_args+=(--api-key-env "$CASE_API_KEY_ENV")
  credential_source="--api-key-env $CASE_API_KEY_ENV"
fi
"$root/docker/run-hewo-e2e.sh" --agent "$name" --backend "$backend" --provider "$provider" --model "$model" \
...
ARTIFACT_PATH="$workspace/artifacts/hewo-smoke.md" \
  "$root/.agents/development/hewo/cases/verifiers/example-verifier.sh"
"$root/.agents/scripts/collect-trace.sh" "$trajectory" "$scrubbed" >/dev/null
...
printf '{"case":"hewo-infrastructure-smoke","evidence_scope":"infrastructure-smoke-only","benchmark":"hewo-infrastructure-smoke","agent":"%s","backend":"%s","provider":"%s","version":"%s","model":"%s","credential_source":"%s","runtime":"%s","image_id":"%s","definition_revision":"%s","task":"%s","artifact":"%s","trajectory":"%s","scrubbed_trajectory":"%s","run_dir":"%s"}\n' \
