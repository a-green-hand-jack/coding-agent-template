...
Deterministic: identical inputs produce byte-identical output. Writes only the
two ``.mmd`` files, never into a product runtime directory. No provider access,
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-readme",
        action="store_true",
        help="write only the .mmd files (the only supported mode in this revision)",
    )
    ...
    if args.check:
        if drift:
            ...
            return 1
        print(f"No drift for {args.agent}: {ARCHITECTURE_BASENAME}, {OPTIMIZATION_BASENAME}")
        return 0
